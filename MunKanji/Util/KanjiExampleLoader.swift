...
class KanjiExampleLoader {
    ...
    func loadKanjiExamplesIfNeeded(modelContext: ModelContext) {
        ...
        for (key, value) in jsonData {
            // JSON 키를 Int로 변환 (예: "0" -> 0)
            guard let kanjiID = Int(key),
                  let kanjiData = value as? [String: Any],
            ...
            print("처리 중: ID=\(kanjiID), 한자=\(kanji)")
            ...
            let kanjiWithExamples = KanjiWithExampleWords(kanjiID: kanjiID, kanji: kanji, examples: examples)
            modelContext.insert(kanjiWithExamples)
            processedCount += 1
        ...
        do {
            ...
            print("KanjiWithExampleWords 데이터가 성공적으로 로드되었습니다.")
            ...
            if let firstKanjiExample = savedData.first {
                print("=== 첫 번째 KanjiWithExampleWords ===")
                print("kanjiID: \(firstKanjiExample.kanjiID)")
